...
const DashboardPage = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Fetch transactions with increased limit for better fallback calculations
      const transactionsRes = await getTransactions({ 
        limit: useSummaryEndpoint ? 50 : 200, // Increased limit for better fallback data
        sortOrder: 'desc',
      ...
      // Set charts loaded only after confirming valid data
      setChartsLoaded(true);
  ...
  // Helper to sum by type with proper number handling
  ...
  // Calculate chart data with optimizations - removed transactions dependency
  const chartData = useMemo(() => {
    ...
    if (summaryData && summaryData.weeklySalesData && isValidArray(summaryData.weeklySalesData)) {
      ...
      // Fallback to calculating from filteredTransactions
      ...
      filteredTransactions
        ...
        .forEach(t => {
          ...
          if (dayIndex >= 0 && dayIndex < 7) {
            salesByDay[dayIndex] += Number(t.amount) || 0;
          }
        });
    ...
    if (summaryData) {
      if (chartToggle === 'customers' && summaryData.topCustomers && isValidArray(summaryData.topCustomers)) {
        topParties = summaryData.topCustomers
          .map((customer, index) => ({
            ...
            value: Number(customer.totalSales) || 0,
        ...
        topParties = summaryData.topSuppliers
          .map((supplier, index) => ({
            ...
            value: Number(supplier.totalPurchases) || 0,
            color: COLORS[index % COLORS.length]
          }))
          .filter(party => party.value > 0);
      }
      ...
      filteredTransactions
        ...
        .forEach(t => {
          ...
          partyMap.set(id, (partyMap.get(id) || 0) + (Number(t.amount) || 0));
        });
  ...
  }, [summaryData, filteredTransactions, chartToggle, suppliers, customers]);
    ...
    <div className="w-full max-w-full overflow-hidden">
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4 sm:mb-6">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600">Business performance overview</p>
          </div>
          <div className="w-full lg:w-auto flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full">
              {/* Small Customer Card */}
              <div className="bg-white p-3 sm:p-4 rounded-lg shadow-md border border-gray-100 hover:shadow-lg transition-shadow">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-base sm:text-lg font-bold text-blue-600">{kpis.totalCustomers}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-3 sm:p-4 rounded-lg shadow-md border border-gray-100 hover:shadow-lg transition-shadow">
                <div className="flex items-center space-x-2 sm:space-x-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-base sm:text-lg font-bold text-orange-600">{kpis.totalSuppliers}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-2 sm:gap-3 w-full">
              ...
              <select
                ...
                className="w-full sm:w-44 px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent text-sm bg-white"
              >
              ...
              <button
                ...
                className={`w-full sm:w-auto text-center px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors ${
                  useSummaryEndpoint 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-6 sm:mb-8">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Total Sales</p>
              <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-green-600 truncate">${kpis.totalSales.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Total Purchase</p>
              <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-red-600 truncate">${kpis.totalPurchase.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Total Payment In</p>
              <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-purple-600 truncate">${kpis.totalPaymentIn.toLocaleString()}</p>
            </div>
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow">
          <div className="flex items-center justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-medium text-gray-500 mb-1">Total Payment Out</p>
              <p className="text-xl sm:text-2xl lg:text-3xl font-bold text-pink-600 truncate">${kpis.totalPaymentOut.toLocaleString()}</p>
            </div>
            <div className="p-2 sm:p-3 bg-pink-100 rounded-full flex-shrink-0">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 text-pink-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
        ...
        <div className="bg-white p-3 sm:p-4 lg:p-6 rounded-xl shadow-lg border border-gray-100">
          <div className="flex items-center justify-between mb-4 sm:mb-6">
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Sales This Week</h3>
          </div>
          <div className="h-64 sm:h-72 lg:h-80 -ml-2 sm:-ml-4 lg:ml-0">
            {!chartsLoaded || chartData.salesData.length === 0 || chartData.salesData.every(item => item.sales === 0) ? (
              <div className="flex items-center justify-center h-full text-gray-500">
                <div className="text-center">
                  <svg className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
                  </svg>
                  <p className="text-sm">No data to be shown</p>
                </div>
              </div>
            ) : (
              <ResponsiveContainer width="100%" height="100%" style={{ outline: 'none' }}>
                <LineChart data={chartData.salesData} margin={{ left: -20, right: 20, top: 5, bottom: 5 }}>
        ...
        <div className="bg-white p-3 sm:p-4 lg:p-6 rounded-xl shadow-lg border border-gray-100">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-6 gap-2">
            ...
            <button
              ...
              className="px-3 sm:px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors font-medium text-sm sm:text-base self-start sm:self-auto"
            >
              Toggle
            </button>
          </div>
          <div className="h-64 sm:h-72 lg:h-80">
            {!chartsLoaded || chartData.topParties.length === 0 || chartData.topParties.every(p => p.value === 0) ? (
              <div className="flex items-center justify-center h-full text-gray-500">
                <div className="text-center">
                  <svg className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-2 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          ...
          {transactions.length > 0 ? (
            ...
            <div className="text-center py-6 sm:py-8">
              ...
              <p className="text-sm sm:text-base text-gray-500">No recent transactions</p>
            </div>
          )}
        </div>
      </div>
